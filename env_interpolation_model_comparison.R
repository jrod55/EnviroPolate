...
# tmp = data.table::fread("../expose/data/sensor_data/za22_data_with_coordinates.txt")
# tmp = tmp %>% 
#   mutate(day = data.table::as.IDate(date)) %>% 
#   filter(day=="2022-01-28") %>% 
#   dplyr::select(-c(day))
# data.table::fwrite(tmp,
#                    paste0("./data/example_sensor_data.txt"),
#                    row.names = FALSE, col.names = TRUE, sep = "\t")


# my_map = data.table::fread("../expose/data/phenoArch_physical_map/XY_to_real_cm_space.txt")
# data.table::fwrite(my_map,
#                    paste0("XY_to_real_cm_space.txt"),
#                    row.names = FALSE, col.names = TRUE, sep = "\t")

# Set number of cores to use with mclapply --------------------------------
...
## Commands to turn individual plots into videos
